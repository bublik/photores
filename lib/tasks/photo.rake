namespace :photo do
  ...
  task :download do
    ...
    #
    harv.storage_dir = "#{Rails.root}/tmp"
    harv.logger = Logger.new("#{Rails.root}/log/image-parser.log")
  ...
  task :import do
    ...
    harv.logger = Logger.new("#{Rails.root}/log/image-comiter.log")
...
task :update_gems do
  ...
  YAML.load_file("#{Rails.root}/config/gem_list.yml").collect do |k|
  ...
  namespace :fixtures do
    ...
    task :dump_references => :environment do
      ...
      dump_tables.each do |table_name|
        i = "000"
        file_name = "#{Rails.root}/test/fixtures/#{table_name}.yml"
